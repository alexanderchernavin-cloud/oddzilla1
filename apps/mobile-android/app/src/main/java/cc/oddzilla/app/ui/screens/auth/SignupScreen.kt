...
fun SignupScreen(onSwitchToLogin: () -> Unit) {
    ...
    Box(
        ...
        Column(
            ...
            Text(
                stringResource(R.string.auth_signup_title),
            ...
            OutlinedTextField(
                ...
                keyboardActions = KeyboardActions(onDone = { keyboard?.hide(); vm.signup() }),
                modifier = Modifier.fillMaxWidth(),
                enabled = !state.submitting,
            )
            state.error?.let { msg ->
            ...
            OzPrimaryButton(
                text = if (state.submitting) "…" else stringResource(R.string.auth_signup),
                onClick = { keyboard?.hide(); vm.signup() },
                modifier = Modifier.fillMaxWidth(),
                enabled = !state.submitting && state.email.isNotBlank() && state.password.length >= 8,
            )
            OzGhostButton(
                text = stringResource(R.string.auth_have_account),
                onClick = { vm.resetForm(); onSwitchToLogin() },
                modifier = Modifier.fillMaxWidth(),
            )
        }
    }
