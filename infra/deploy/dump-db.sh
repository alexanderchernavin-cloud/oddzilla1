...
#
# Retention here is independent of /var/backups/oddzilla — keep only the
# last 1 pre-deploy snapshot. The /var/backups/oddzilla cron is the
# durable history line; this dir only needs to support "roll back the
# deploy I just did", and at ~6 GB/dump (the DB grew past the old ~3 GB
# estimate) holding two starves the disk — the 2026-06-17 fill had 14 GB
# sitting in two stale pre-deploy dumps. One is enough for that purpose.
