...
6. If the diff includes any `packages/db/migrations/*.sql`: take a pre-deploy
   `pg_dump` to `.deploy/backups/<sha>.sql.gz` (keep last 1, lowered from 2 on
   2026-06-17 — at ~6 GB/dump two of them were starving the disk; override via
   `PRE_DEPLOY_BACKUP_RETENTION`).
