...
| catalogue table + sub-event | `markets.provider_market_id` from `provider_market_types`                                                             | OUR id per (table, sub-event, double-chance), allocated from 3 000 000 (migration `20260908T115542`). One table is reused across every sub-event, so the table alone named three different markets; the readable key is `market_kind` (`fb:120@100201`). Opaque — resolve through the registry, never by arithmetic                                                      |
...
| per-team table (by number) | market NAME carries the team | Fonbet numbers the sides instead of naming them — `Team 1 totals {threshold}` / `Инд. тоталы-1`, `Team Totals-1`, `1 to win` / `Победа 1`, and a couple of captions where its `%1` / `%2` placeholder leaks into the name. `applyTeamNumberLabel` (services/api/src/lib/market-naming.ts) swaps the number for the team at render time — 1 is home, 2 is away, the same convention the factor ids use — so the storefront and the bet slip read "Swansea totals 2.5". Market names only: an outcome caption is left alone by that rule, since the header above it already names the team, and Fonbet writes some of them mid-sentence where a swap reads as broken grammar. |
| per-team COLUMN (by number) | outcome template `home` / `away` | A line table (handicap, total) captions its two team columns "1" and "2" — which is how `sideCaptions` reads them when it keys the outcomes `h1` / `h2` — and a match-winner table's factor ids carry the same captions for sub-event markets. Rendered verbatim, a handicap ladder was headed by two bare numbers and the bet-slip leg said "1" for a team. Since 2026-09-07 `OutcomeTemplate` emits the bare side word for both, exactly as it already did for the `%1` / `%2` captions, so `renderOutcomeLabel` resolves the team in every locale. Guarded on the caption being ONLY the number: two multi-row tables prefix their row ("1st substitution 1", "Handicap from 1 to 6 min 2"), where the row is the outcome's whole identity, and those keep it. Double-chance cells keep "1X" / "X2" / "12" — no single team names them. |
