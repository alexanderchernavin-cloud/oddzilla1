...
// synthesised from Bifrost; we hand the body to the same settler.Handle
// the AMQP consumer uses, so a synthesised bet_settlement goes through the
// identical apply-once insert, outcome cascade and per-ticket payout path.
// Odds-family kinds are ignored here (feed-ingester has its own consumer
// group on the same stream), exactly as they are on AMQP.
//
...
// log — Bifrost is state-based, so bifrost-feed's next resync re-emits any
// settlement our DB still lacks, and nothing is permanently lost.
//
// Intentionally duplicated in services/feed-ingester/internal/backupstream
