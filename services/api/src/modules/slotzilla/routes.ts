      ...
      const load = async (): Promise<{ games: SlotzillaLiveGame[] }> => {
        const rows = await app.db
          .select({
            ...
            periodSeconds: slotzillaGames.periodSeconds,
            overtimeSeconds: slotzillaGames.overtimeSeconds,
            regulationPeriods: slotzillaGames.regulationPeriods,
      ...
      const load = async (): Promise<{ games: SlotzillaLiveGame[] }> => {
        ...
        const view = (r: (typeof rows)[number], demo: boolean) => ({
          ...
          clock: clockView({
            ...
            periodSeconds: r.periodSeconds ?? null,
            overtimeSeconds: r.overtimeSeconds ?? null,
            regulationPeriods: r.regulationPeriods ?? null,
          }),
          score: scoreOf(r.liveScore),
          playerMode: r.coverageLevel === 2,
          scheduledAt: r.scheduledAt?.toISOString() ?? null,
