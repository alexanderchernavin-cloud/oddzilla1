...
func fixtureIndex() *fonbet.Index {
	cat := &fonbet.Catalog{Groups: []fonbet.Group{{Name: "Основные ставки", Tables: []fonbet.Table{
		...
		{Num: 304, Name: "Фора", IsMain: true, SortByParam: true, Rows: [][]fonbet.Cell{
			{{Name: ""}, {Name: "1"}, {Name: ""}, {Name: "2"}},
			{{Kind: "param", FactorID: 910}, {Kind: "value", FactorID: 910}, {Kind: "param", FactorID: 912}, {Kind: "value", FactorID: 912}},
		}},
		{Num: 305, Name: "Тотал", IsMain: true, SortByParam: true, Rows: [][]fonbet.Cell{
			{{Name: ""}, {Name: "Б"}, {Name: "М"}},
...
func TestGradeFootball(t *testing.T) {
	...
	outs, ok, why := Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "3", "2"}}, idx, 1000000, 1900000, "", 1, ss)
	if !ok || len(outs) != 3 || outs[0] != (Outcome{"1", "1", ""}) || outs[1] != (Outcome{"2", "0", ""}) || outs[2] != (Outcome{"3", "0", ""}) {
		t.Fatalf("winner: %+v %v %s", outs, ok, why)
	}
	outs, ok, _ = Grade(Market{PMID: 1900120, Specs: map[string]string{}, OutcomeIDs: []string{"924", "1571", "925"}}, idx, 1000000, 1900000, "", 1, ss)
	if !ok || outs[0] != (Outcome{"1571", "1", ""}) || outs[1] != (Outcome{"924", "1", ""}) || outs[2] != (Outcome{"925", "0", ""}) {
		t.Fatalf("double chance: %+v", outs)
	}
	outs, ok, _ = Grade(Market{PMID: 1000304, Specs: map[string]string{"handicap": "-0.5"}, OutcomeIDs: []string{"h1", "h2"}}, idx, 1000000, 1900000, "", 1, ss)
	if !ok || outs[0] != (Outcome{"h1", "1", ""}) || outs[1] != (Outcome{"h2", "0", ""}) {
		t.Fatalf("handicap: %+v", outs)
	}
	// 1st half total 2.5 → 2 goals → under
	outs, ok, _ = Grade(Market{PMID: 1000305, Specs: map[string]string{"threshold": "2.5", "variant": "fb:100201"}, OutcomeIDs: []string{"over", "under"}}, idx, 1000000, 1900000, "1-й тайм", 1, ss)
	...
	outs, ok, _ = Grade(Market{PMID: 1000305, Specs: map[string]string{"threshold": "1.5", "side": "home"}, OutcomeIDs: []string{"over", "under"}}, idx, 1000000, 1900000, "", 1, ss)
	if !ok || outs[0] != (Outcome{"over", "1", ""}) {
		t.Fatalf("team total: %+v", outs)
	}
	// OT table is skipped, unknown sport is skipped, missing stat is skipped
	if _, ok, _ = Grade(Market{PMID: 1000400, Specs: map[string]string{}, OutcomeIDs: []string{"5001", "5002"}}, idx, 1000000, 1900000, "", 2, ss); ok {
		t.Fatalf("OT table must be skipped")
	}
	if _, ok, _ = Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "3", "2"}}, idx, 1000000, 1900000, "", 11632, ss); ok {
		t.Fatalf("darts must be skipped")
	}
	if _, ok, _ = Grade(Market{PMID: 1000305, Specs: map[string]string{"threshold": "4.5", "variant": "fb:1"}, OutcomeIDs: []string{"over", "under"}}, idx, 1000000, 1900000, "желтые карты", 1, ss); ok {
...
func TestGradeTennisAndOT(t *testing.T) {
	...
	outs, ok, why := Grade(Market{PMID: 1000304, Specs: map[string]string{"handicap": "-3.5"}, OutcomeIDs: []string{"h1", "h2"}}, idx, 1000000, 1900000, "", 4, tennis)
	if !ok || outs[0].Result != "1" {
		t.Fatalf("tennis games handicap: %+v %s", outs, why)
	}
	// games total 21 → 21.5 under
	outs, ok, _ = Grade(Market{PMID: 1000305, Specs: map[string]string{"threshold": "21.5"}, OutcomeIDs: []string{"over", "under"}}, idx, 1000000, 1900000, "", 4, tennis)
	if !ok || outs[1].Result != "1" {
		t.Fatalf("tennis games total: %+v", outs)
	}
	// winner by sets
	outs, ok, _ = Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "2"}}, idx, 1000000, 1900000, "", 4, tennis)
	if !ok || outs[0].Result != "1" {
	...
	outs, ok, why = Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "2"}}, idx, 1000000, 1900000, "", 3, bb)
	...
	if _, ok, _ = Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "2"}}, idx, 1000000, 1900000, "", 2, hk); ok {
		t.Fatalf("tied two-way hockey must stay open")
	}
	// ... but a three-way winner settles as a draw
	outs, ok, _ = Grade(Market{PMID: 1000120, Specs: map[string]string{}, OutcomeIDs: []string{"1", "3", "2"}}, idx, 1000000, 1900000, "", 2, hk)
