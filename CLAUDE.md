...
| Admin: team logos | Live (2026-05-05) | `/admin/competitors` lists every team with sport/q/missing-logo filters, inline edit of `logo_url` / `brand_color` / `abbreviation`, and a per-row Clear button. Backed by [`services/api/src/modules/admin/competitors.ts`](./services/api/src/modules/admin/competitors.ts) (PATCH /admin/competitors/:id + audit logging) and the bulk endpoint POST /admin/competitors/bulk-logos used by [`packages/db/src/apply-logo-seed.ts`](./packages/db/src/apply-logo-seed.ts) (curated overrides for popular teams in [`packages/db/seeds/competitor-logos.json`](./packages/db/seeds/competitor-logos.json)). For exhaustive coverage, [`packages/db/src/resolve-logos.ts`](./packages/db/src/resolve-logos.ts) iterates every active competitor across the 15 supported esports (cs2/lol/valorant/dota2/ml/kog/r6/sc2/cod/aov/rocketleague/overwatch/crossfire/sc1/w3) and bulk-queries Liquipedia's MediaWiki API (`prop=pageimages&piprop=original`, batches of 40, 2.1 s between batches) — runs server-side via `pnpm --filter @oddzilla/db db:resolve-logos`. The catalog API surfaces `homeLogoUrl` / `awayLogoUrl` on every match row + match detail; `TeamMark` renders the image with an `onError` fallback to the existing initials block, so a stale URL or unbranded team degrades gracefully. |
