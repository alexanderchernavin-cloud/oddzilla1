...
| Stale-ticket sweeper (recovery + 48h void) | `services/settlement/internal/sweeper/sweeper.go`; `services/settlement/internal/store/stale_tickets.go`. Periodic loop (default 30 min) that fires `pg_notify('fixture_refresh', urn)` for matches with stuck `accepted` tickets in the 5h–48h window, and voids+refunds stale legs once a ticket's match is past the 48h cut-off. Uses settler's exported `MaybeSettleTicketTx` so payout math, wallet movement, and ref_id generation match the AMQP-driven path. |
...
| --- | --- | --- |
| DB schema + migrations | Live | 19 migrations 0000–0018. See "SQL migrations" row above for the full list. Recent additions: `0013_tournament_risk_tier`, `0014_multi_currency` (per-currency wallets + OZ demo), `0015_cashout`, `0016_cashout_acceptance_delay`, `0017_tiple_tippot`, `0018_bet_product_per_leg_margin`. |
...
| Settlement: stale-ticket sweeper | Live | `services/settlement/internal/sweeper/` runs every 30 min (env `SETTLEMENT_STALE_SWEEP_INTERVAL`). Two phases: (1) for tickets still `accepted` whose match scheduled_at is between **5h** and **48h** in the past, fire `pg_notify('fixture_refresh', urn)` per match URN — feed-ingester re-pulls the fixture from REST so `matches.status` reflects reality; (2) for tickets whose stuck legs are past **48h**, void the stale selections (`result=void, void_factor=1`) and re-run `MaybeSettleTicketTx` — singles refund stake, combos drop the void leg to a 1× multiplier so the rest of the ticket can still settle. Admin audit row written per voided ticket; user gets a Redis-fanned ticket frame so the open session updates. Thresholds tunable via `SETTLEMENT_STALE_RECOVERY_AGE_HOURS` / `SETTLEMENT_STALE_VOID_AGE_HOURS`. |
