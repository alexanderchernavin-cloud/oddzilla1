...
| Android client (Kotlin + Compose) | [`apps/mobile-android/`](./apps/mobile-android/) — native storefront-only Android app distributed as a self-hosted APK (never published to Play Store). Manual-DI service locator (`common/Deps.kt`), single Activity (`MainActivity.kt`), bottom-tab scaffold (Sports / Bets / Wallet / Account in `ui/screens/main/MainScaffold.kt`). REST via Retrofit + OkHttp with `PersistentCookieJar` (DataStore-backed) so accessToken + refreshToken survive process death; `OriginInterceptor` stamps `Origin: https://oddzilla.cc` so the API's CSRF plugin lets the app through with zero API changes. `TokenAuthenticator` catches 401 and refreshes once before retrying. Self-update flow lives in `update/`: `UpdateController` polls `https://oddzilla.cc/app/version.json` on every cold start, compares against `BuildConfig.VERSION_CODE`, downloads the new APK via OkHttp into `externalCacheDir/updates/`, verifies SHA-256 against the manifest, and hands off to `Intent.ACTION_VIEW + application/vnd.android.package-archive` via `FileProvider`. Mandatory updates remove the Later button. Version source-of-truth is `version.properties` at the project root; `scripts/release.ps1` (and the bash twin) builds a signed APK, hashes it, scps both the APK + new `version.json` to `team@178.104.174.24:/srv/oddzilla-apk/`. Caddy serves that directory at `/app/*` on the apex (see Caddyfile + docker-compose.yml caddy.volumes mount). Splash screen via the AndroidX SplashScreen API. Compose theme ports the web design tokens verbatim (`ui/theme/Color.kt` ↔ `apps/web/src/app/globals.css`); extended palette (positive / negative / live / tier-gold) reaches composables via `OzTheme.colors` (`LocalOzColors` CompositionLocal). |
...
| Android client (storefront only) | Live: full storefront + community + drift accept + sport-filter pills + push device registration + GH Actions CI | [`apps/mobile-android/`](./apps/mobile-android/) — Kotlin + Compose + Material 3, native rewrite (not WebView). Self-distributed APK with a built-in update flow that polls `/app/version.json`, compares versionCode, downloads + SHA-256 verifies + hands off to the system installer. **Working today (v0.4.0)**: 5-tab bottom-nav (Sports / Community / Bets / Wallet / Account), Sports list → Sport detail (matches with inline match-winner odds, pull-to-refresh) → Match detail with **live scoreboard** ([`LiveScoreboard.kt`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/screens/match/LiveScoreboard.kt) parses `matches.live_score` JSON for per-map score + current-map highlight), **stream embed** ([`MatchStreamEmbed.kt`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/screens/match/MatchStreamEmbed.kt) — small WebView wrapper that classifies Twitch / YouTube / Kick / Gjirafa URLs into proper iframe embeds, with a pill toggle when the match has multiple feeds), and **line-market grouping** (Totals + Handicaps with the same `lineKey` collapse into one card with one row per line value). Markets group by scope tabs (Match / Map 1 / Map 2 / Top). **Live odds WebSocket** to `wss://oddzilla.cc/ws` ([`LiveOddsClient`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/data/ws/LiveOddsClient.kt)) refcounted across screens, ticks override published odds in real time, exponential backoff up to 15 s on reconnect, ticket frames trigger the Bets tab's auto-refresh on placement. **Bet slip** ([`BetSlipController`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/bet/BetSlipController.kt)) cross-match (max 20), Single vs Combo, USDC/OZ currency switch persisted in DataStore, idempotency-key UUID per placement, ModalBottomSheet UI ([`BetSlipSheet`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/screens/bet/BetSlipSheet.kt)) reachable from a slip-count chip in the MainScaffold top bar and from MatchDetailScreen's peek bar. **Bets tab** Active/Settled tabs, per-active-ticket Cash out (quote-then-accept pattern with `expectedOfferMicro` round-trip). **Wallet tab** balances per currency, ERC20 deposit address copy + tx-hash submission for deposit intents, USDC withdrawal request form. **Community tab** ([`CommunityFeedScreen`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/screens/community/CommunityFeedScreen.kt)) Recent / Best Wins tabs + USDC/OZ/All currency filter + cursor pagination, taps push to [`CommunityProfileScreen`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/screens/community/CommunityProfileScreen.kt) (avatar monogram, bio, per-currency stats card, achievements grid, recent bets). **Copy-this-bet** button on settled / cashed-out / accepted cards calls `POST /community/copy/:id` and merges the available legs into the current slip via `BetSlipController.add()`, surfaced via the central [`SnackbarController`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/common/Snackbar.kt). Pull-to-refresh ([`OzPullToRefresh`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/components/OzPullToRefresh.kt)) wraps every fetched-list screen; [`EmptyState`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/ui/components/EmptyState.kt) is the standard empty/error chrome. **Bet slip drift accept** (v0.4.0): `BetSlipController` watches `LiveOddsClient.odds` for every selection; on divergence sets `pendingOdds` per leg, the sheet shows an "X.XX → Y.YY" delta, and "Place bet" is replaced with "Accept new odds" until the user opts in (`acceptPendingOdds()` copies pending → odds across all legs). Subsequent ticks that match the user-accepted price clear the pending automatically — broker bouncing through the same value is a no-op. **Sport-filter pills** added to the community feed: pulls `/catalog/sports` once and surfaces a horizontally-scrollable pill row above the existing currency + tab filters; selection threads through as the `?sport=` query on the feed endpoint. **Push-notification scaffolding (server-side live, client commented-out)** — migration [`0045_user_devices`](./packages/db/migrations/0045_user_devices.sql) + [`devices/routes.ts`](./services/api/src/modules/devices/routes.ts) provide `POST /devices/register` (idempotent upsert, revokes the same token on any other user for hand-me-down phone safety), `POST /devices/unregister` (soft revoke on logout), and `GET /devices`. Mobile [`DevicesRepository`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/data/repo/DevicesRepository.kt) is wired but the FCM SDK is intentionally not added until a Firebase project exists; full setup walkthrough at [`apps/mobile-android/app/src/main/java/cc/oddzilla/app/fcm/README.md`](./apps/mobile-android/app/src/main/java/cc/oddzilla/app/fcm/README.md). Push-sender on the API (Firebase Admin SDK integration that reads `user_devices` and calls `sendEachForMulticast()` from settlement / cashout / community emit sites) is the documented next step. **Brand TTFs** (Geist + Instrument Serif) drop-in via [`res/font/README.md`](./apps/mobile-android/docs/fonts.md); current build falls back to system fonts. **CI**: [`.github/workflows/android.yml`](./.github/workflows/android.yml) — push to main builds debug APK as a workflow artifact; `workflow_dispatch -release=true` signs with keystore restored from GH secrets and scps the result to `/srv/oddzilla-apk/`, mirroring `scripts/release.ps1`. The update modal (Available / Downloading / ReadyToInstall / Failed with mandatory variant) overlays everything else. Stubbed for follow-up sessions: FCM client (Firebase project + uncomment 5 lines per the fcm/README), API push-sender, real launcher artwork, real fonts, analyses + competitions + notifications panel surfaces from the web Phase 10.5/11/12. Release flow: `apps/mobile-android/scripts/release.ps1` (or `.sh`, or the GH Actions release dispatch) builds a signed APK, scps to `/srv/oddzilla-apk/oddzilla-<version>.apk`, updates `version.json`. Caddy reverse-proxies the dir at `https://oddzilla.cc/app/*` with `Cache-Control: no-store` on the JSON manifest so update prompts propagate within minutes. |
