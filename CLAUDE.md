...
| Recovery + alive-gap + handover sweeper | `services/feed-ingester/internal/handler/handler.go` (`TriggerRecovery`, `AliveState`, `mapFixtureStatus`, `handleMatchStatusChange`); `services/feed-ingester/cmd/feed-ingester/main.go` (`runHandoverSweeper`, `runRecoveryListener`, `runFixtureRefreshListener`, `runPhantomDrainListener`, `runPhantomDrainTicker`). Three NOTIFY channels: `feed_recovery` (full AMQP replay), `fixture_refresh` (single-URN REST refetch on phantom-stale trip wire), `phantom_drain` (full `DrainPhantomStale` sweep over both `live` and `not_started` matches stuck past their scheduled start, paced 5 RPS — fired by the admin button, the on-boot ticker, and the hourly ticker). |
...
| Recovery flow | Live | `POST /v1/{product}/recovery/initiate_request` triggered on every (re)connect for both producers + on `alive subscribed=0` + on `alive` timestamp drift > 5s. Admin `POST /admin/feed/recovery` rewinds the cursor and fires both `feed_recovery` (AMQP replay) and `phantom_drain` (REST fixture refetch for matches stuck at `live` or `not_started` older than `drainAgeHours`, default 6 h — Oddin's broker often skips `match_status_change` entirely, so this is the authoritative cleanup). `runPhantomDrainTicker` fires `phantom_drain` once 30 s after boot and then every hour (age=6 h), so phantom cleanup no longer waits on a manual click and a fresh ingester catches up immediately; the listener's mutex guarantees the ticker and admin click never collide. |
