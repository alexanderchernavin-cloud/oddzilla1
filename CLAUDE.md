...
scraper was cancelled during Phase 8 (migration 0003). As of 2026-04-18
the site is live at `s.oddzilla.cc` with the new design, market
whitelist active, and `payback_margin_bp=0` globally (Oddin already
sends margined odds — see [`docs/OPERATIONS.md`](./docs/OPERATIONS.md)
before re-enabling). Migration 0032 (2026-05-07) replaced the per-user
HD-address USDT flow with a single shared ERC20 address + user-submitted
tx-hash attribution; wallet-watcher is now intent-driven. Next:
pre-launch exit gates (KYC, wallet reconciliation, off-server backup
rsync, monitoring, runbook). Full roadmap in
