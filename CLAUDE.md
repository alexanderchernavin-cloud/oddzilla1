...
| UI primitives | [`apps/web/src/components/ui/`](./apps/web/src/components/ui/) — `primitives.tsx` (Button, Pill, LiveDot, Tabs, OddButton, TeamMark, Divider), `icons.tsx`, `monogram.tsx`, `sport-glyph.tsx`, `tier-mark.tsx` (gold "Top" star + `isFeaturedTier()` predicate for Oddin `risk_tier ∈ {1,2}`; rendered next to the tournament name on match cards, the sidebar tournament tree, top-bar search hits, and the match-detail header. Featured cards also pick up a 2px gold left-edge accent and a "TOP" pill on the detail page; both tiers share one visual treatment per the user's "просто Top для обоих этих тиров" call) |
...
| --- | --- | --- |
| DB schema + migrations | Live | 20 migrations 0000–0019. See "SQL migrations" row above for the full list. Recent additions: `0013_tournament_risk_tier`, `0014_multi_currency` (per-currency wallets + OZ demo), `0015_cashout`, `0016_cashout_acceptance_delay`, `0017_tiple_tippot`, `0018_bet_product_per_leg_margin`, `0019_fe_market_display_order` (per-sport storefront ordering of market types). |
...
| Admin: FE Settings (markets display order) | Live | `/admin/fe-settings/markets-order` per-sport editor — pick a sport, drag market types between Ordered and Unranked columns or reorder via up/down buttons. PUT `/admin/fe-settings/markets-order/:sportId` writes a transactional replace into `fe_market_display_order`. `/catalog/matches/:id` reads the table per request and sorts each scope group: configured ids first by `display_order`, then unranked by `provider_market_id` ascending (the legacy default). |
