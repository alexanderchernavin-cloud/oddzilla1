...
| Admin: team logos | Live (2026-05-06) | `/admin/competitors` lists every team with sport/q/missing-logo filters, inline edit of `logo_url` / `brand_color` / `abbreviation`, and a per-row Clear button. Backed by [`services/api/src/modules/admin/competitors.ts`](./services/api/src/modules/admin/competitors.ts) (PATCH /admin/competitors/:id + audit logging). **Logos are self-hosted, never hot-linked.** Docker named volume `team-logos` mounted into `api` (RW at `/data/team-logos`) and `caddy` (RO at `/srv/team-logos`); Caddy serves it at `/team-logos/*` with `Cache-Control: public, max-age=31536000, immutable`. The single resolver, [`packages/db/src/resolve-logos.ts`](./packages/db/src/resolve-logos.ts), joins `competitors` to `competitor_profiles.icon_path` (populated by feed-ingester from Oddin's `/v1/sports/{lang}/competitors/{urn}/profile` endpoint), downloads each `cdn.oddin.gg/...` icon to the volume, and writes `/team-logos/{competitor_id}.{ext}` back to `competitors.logo_url`. Run via `pnpm --filter @oddzilla/db db:resolve-logos` — ~1800 teams resolves in ~30 s on prod. Coverage on the current feed: 1861/2135 cached profiles have icon_path set (~87%); rows without one render the `TeamMark` initials fallback. The catalog API surfaces `homeLogoUrl` / `awayLogoUrl` on every match row + match detail; `TeamMark` falls back to initials on `<img onError>` so a stale path never breaks the layout. |
