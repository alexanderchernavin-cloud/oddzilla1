...
| Recovery flow | Live | `POST /v1/{product}/recovery/initiate_request` triggered on every (re)connect for both producers + on `alive subscribed=0` + on `alive` timestamp drift > 5s. Admin `POST /admin/feed/recovery` rewinds the cursor and fires both `feed_recovery` (AMQP replay) and `phantom_drain` (REST fixture refetch for stuck-live matches older than `drainAgeHours`, default 6 h). `runPhantomDrainTicker` also fires `phantom_drain` once per hour automatically (age=6 h) so phantom-live cleanup no longer waits on a manual click; the listener's mutex guarantees the ticker and admin click never collide. |
| `match_status_change` handler | Live | `handleMatchStatusChange` in `services/feed-ingester/internal/handler/handler.go` decodes `<match_status_change status="N"/>` and flips `matches.status` per `oddinxml.MapMatchStatusCode` (UOF convention: 1→live, 3/4→closed, 5/9→cancelled, etc.). Oddin's integration broker emits these only sparsely — many matches end without one — so the handler is a fast path; the hourly REST drain remains the authoritative cleanup. Settlement service recognizes the kind and ignores it (its domain is bet_settlement / bet_cancel). |
