...
| Phantom-drain ticker (safety net) | Live | Catches matches Oddin's broker simply never sent any terminal signal for. `runPhantomDrainTicker` fires `phantom_drain` once 30 s after boot and then **every 15 min** (`drainAgeHours=0`); `Resolver.DrainPhantomStale` re-fetches each candidate via REST `/sport_events/{urn}/fixture` and applies whatever status the fixture reports through `UpsertMatch`. Some phantoms are stale in REST too — those are noise we live with, not bugs to fix. |
