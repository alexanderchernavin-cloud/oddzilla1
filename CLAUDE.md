...
| Recovery flow | Live | `POST /v1/{product}/recovery/initiate_request` triggered on every (re)connect for both producers + on `alive subscribed=0` + on `alive` timestamp drift > 5s. **Suspend-before-recover is unconditional (2026-05-11):** every AMQP (re)connect runs `store.FlushAndSuspendActiveCatalog` BEFORE asking Oddin to replay — drops orphan markets/matches (no money attached → cascading delete), suspends the surviving (money-attached) ones to `status=-1` with null odds/probability. The replay re-activates only what Oddin re-confirms; anything Oddin omits stays suspended and drops from listings via the catalog filter. Rationale: even a few seconds of feed gap can leave a market quoting odds Oddin already moved off of, and a single placement at stale odds is real-money loss. The 2026-05-09 disk-full incident wedged 33 matches this way (Oddin's replay didn't carry their terminal status because they were no longer in Oddin's active state by reconnect time). The recovery request itself clamps the cursor to `now - 24 h` (per `handler.recoveryWindowCap`) to stay in Oddin's lenient rate-limit tier (< 30 min) and bound replay latency; anything older falls through as "stayed suspended". Admin `POST /admin/feed/recovery` (with `flushOdds=true` default) is the same flush mechanism plus a forced cursor rewind for operator-initiated catalog rebuilds. |
