...
| Recovery + alive-gap + handover sweeper | `services/feed-ingester/internal/handler/handler.go` (`TriggerRecovery`, `AliveState`, `mapFixtureStatus`, `handleMatchStatusChange`); `services/feed-ingester/cmd/feed-ingester/main.go` (`runHandoverSweeper`, `runRecoveryListener`, `runFixtureRefreshListener`, `runPhantomDrainListener`, `runPhantomDrainTicker`). Three NOTIFY channels: `feed_recovery` (full AMQP replay), `fixture_refresh` (single-URN REST refetch on phantom-stale trip wire), `phantom_drain` (full `DrainPhantomStale` sweep over both `live` and `not_started` matches stuck past their scheduled start, paced 5 RPS — fired by the admin button, the on-boot ticker, and the 15-minute ticker). |
...
| Match lifecycle apply (forward-only) | `services/feed-ingester/internal/store/catalog.go` (`UpdateMatchStatus` — SQL guard rejects regressions from `closed`/`cancelled` and to `not_started`; `UpdateAllMarketsStatusForMatch` for `bet_stop groups="all"`); `services/feed-ingester/internal/handler/handler.go` (`handleOddsChange` applies terminal codes from `<sport_event_status status="…"/>`; `handleBetStop` applies blanket suspension; `handleMatchStatusChange` unchanged); `services/settlement/internal/store/store.go` (`MarkMatchClosedByURN` — same forward-only guard, called from `handleBetSettlement` when `certainty=2`). The phantom-drain in `services/feed-ingester/internal/automap/resolver.go` (`DrainPhantomStale`) remains as the safety net via REST refetch. |
...
| --- | --- | --- |
| DB schema + migrations | Live | 22 migrations 0000–0021. See "SQL migrations" row above for the full list. Recent additions: `0013_tournament_risk_tier`, `0014_multi_currency` (per-currency wallets + OZ demo), `0015_cashout`, `0016_cashout_acceptance_delay`, `0017_tiple_tippot`, `0018_bet_product_per_leg_margin`, `0019_fe_market_display_order` (per-sport storefront ordering of market types), `0020_fe_market_order_scope` (per-scope ordering: match/map/top + Top tab), `0021_competitor_logos` (`competitors.logo_url` + `brand_color` for storefront team branding). |
...
| Recovery flow | Live | `POST /v1/{product}/recovery/initiate_request` triggered on every (re)connect for both producers + on `alive subscribed=0` + on `alive` timestamp drift > 5s. Admin `POST /admin/feed/recovery` rewinds the cursor and fires both `feed_recovery` (AMQP replay) and `phantom_drain` (REST fixture refetch for matches stuck at `live` or `not_started` past their scheduled start by `drainAgeHours`, **default 0 h** — Oddin's broker often skips `match_status_change` entirely, so REST is the authoritative state and waiting longer just leaves stale rows in listings). `runPhantomDrainTicker` fires `phantom_drain` once 30 s after boot and then **every 15 min** (age=0 h), so a fresh match catches up within ~15 min of its actual start; the listener's mutex guarantees the ticker and admin click never collide. |
| `match_status_change` handler | Live | `handleMatchStatusChange` in `services/feed-ingester/internal/handler/handler.go` decodes `<match_status_change status="N"/>` and flips `matches.status` per `oddinxml.MapMatchStatusCode` (UOF convention: 1→live, 3/4→closed, 5/9→cancelled, etc.). Oddin's integration broker emits these only sparsely — many fixtures transition `not_started` → `closed` without ever passing through `live` in the AMQP stream. Three other in-band sources now flip lifecycle state so the broker's silence stops mattering: (1) `odds_change` carries `<sport_event_status status="…"/>` and we apply terminal codes (`closed`/`cancelled`) from it via the same `UpdateMatchStatus`; (2) `bet_stop groups="all"` blanket-suspends every non-terminal market on the event; (3) `bet_settlement certainty=2` flips `matches.status='closed'` via settlement's `MarkMatchClosedByURN`. All paths use a forward-only SQL guard so a stale post-game odds_change can't reactivate a closed match. The 15-min phantom-drain ticker remains as a backstop for fixtures none of those signals reach. Settlement service recognizes the kind and ignores it (its domain is bet_settlement / bet_cancel). |
| Embedded `<sport_event_status>` lifecycle | Live (2026-05-06) | `handleOddsChange` decodes `<sport_event_status status="…"/>` (already used for the live scoreboard) and calls `UpdateMatchStatus` when the Oddin code maps to `closed` or `cancelled`. Non-terminal codes are skipped to avoid flapping per odds tick; the forward-only SQL guard inside `UpdateMatchStatus` (rejects regressions from terminal status and to `not_started`) covers the remaining edge cases. Closes the gap where Oddin's broker stops emitting `match_status_change` for esports. |
| `bet_stop` blanket suspension | Live (2026-05-06) | `handleBetStop` flips every non-terminal market (`status NOT IN (-3, -4)`) on the event when `groups="all"`, via new store helper `UpdateAllMarketsStatusForMatch`. Group-specific `bet_stop` (e.g. `groups="main_markets"`) needs `market_descriptions` REST data and is still log-and-skip. Settled (-3) and cancelled (-4) markets are deliberately preserved — those are owned by the settlement service. |
| `bet_settlement certainty=2` closes match | Live (2026-05-06) | After applying every market in a `bet_settlement`, `handleBetSettlement` checks `certainty=2` (post-game) and calls `store.MarkMatchClosedByURN(event_urn)` with a forward-only SQL guard that skips already-terminal matches. This is the authoritative match-end signal because Oddin's integration broker sends `bet_settlement` reliably even when it never emits `match_status_change`. |
| Live-stream embeds (Twitch / YouTube) | Live (2026-05-06) | Oddin's `/v1/sports/{lang}/sport_events/{matchURN}/fixture` returns a `<tv_channels><tv_channel name=… language=… stream_url=…/></tv_channels>` block per the public schema. `oddinxml.FixtureTvChannelsList` parses it; resolver `merge()` JSON-encodes the rows into `matches.tv_channels` (migration 0022). Fixture `STREAM_URL` change_type 106 now also triggers `RefreshFromFixture` so the column tracks broadcaster swaps mid-event. `/catalog/matches/:id` classifies each entry into `{platform: twitch \| youtube \| other, embedId, url, name, language}` and surfaces it as `match.streams`. The storefront `apps/web/src/components/match/match-streams.tsx` renders an iframe (Twitch via `https://player.twitch.tv/?channel=…&parent=<host>`, YouTube via `youtube-nocookie.com/embed/…`) with a tab strip when there are multiple feeds. Caddy's `web_csp` whitelists `frame-src https://player.twitch.tv https://www.twitch.tv https://www.youtube.com https://www.youtube-nocookie.com`. |
...
| Admin: team logos | Live (2026-05-05) | `/admin/competitors` lists every team with sport/q/missing-logo filters, inline edit of `logo_url` / `brand_color` / `abbreviation`, and a per-row Clear button. Backed by [`services/api/src/modules/admin/competitors.ts`](./services/api/src/modules/admin/competitors.ts) (PATCH /admin/competitors/:id + audit logging). **Logos are self-hosted, not hot-linked.** Docker named volume `team-logos` mounted into `api` (read-write at `/data/team-logos`) and `caddy` (read-only at `/srv/team-logos`); Caddy serves it at `/team-logos/*` with `Cache-Control: public, max-age=31536000, immutable`. Resolver scripts download each asset, write `{LOGO_STORE_DIR}/{competitor_id}.{ext}` (extension picked from Content-Type), and store the local path `/team-logos/{id}.{ext}` on `competitors.logo_url`. Three scripts share [`packages/db/src/lib/logo-store.ts`](./packages/db/src/lib/logo-store.ts): `db:resolve-logos` (Liquipedia og:image scrape across 15 supported esports — opensearch fallback, cooperative 429 backoff), `db:apply-logo-seed` (curated overrides from [`packages/db/seeds/competitor-logos.json`](./packages/db/seeds/competitor-logos.json)), `db:localize-logos` (one-shot fix-up for rows still pointing at third-party hosts). The catalog API surfaces `homeLogoUrl` / `awayLogoUrl` on every match row + match detail; `TeamMark` renders the image with an `onError` fallback to initials. |
