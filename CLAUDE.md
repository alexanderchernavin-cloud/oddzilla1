...
| Recovery + alive-gap + handover sweeper | `services/feed-ingester/internal/handler/handler.go` (`TriggerRecovery`, `AliveState`, `mapFixtureStatus`); `services/feed-ingester/cmd/feed-ingester/main.go` (`runHandoverSweeper`, `runRecoveryListener`, `runFixtureRefreshListener`, `runPhantomDrainListener`). Three NOTIFY channels: `feed_recovery` (full AMQP replay), `fixture_refresh` (single-URN REST refetch on phantom-live trip wire), `phantom_drain` (full DrainPhantomLive sweep, paced 5 RPS). |
...
| Recovery flow | Live | `POST /v1/{product}/recovery/initiate_request` triggered on every (re)connect for both producers + on `alive subscribed=0` + on `alive` timestamp drift > 5s. Admin `POST /admin/feed/recovery` rewinds the cursor and fires both `feed_recovery` (AMQP replay) and `phantom_drain` (REST fixture refetch for stuck-live matches older than `drainAgeHours`, default 6 h). The drain handles matches Oddin won't replay match_status_change for (anything beyond ~3 days). |
