// resolve-oddin-icons.ts
//
// Bulk-resolve team logos from Oddin's own competitor profile cache.
...
// `competitor_profiles.icon_path` for every team that turns up in the
// match feed (see services/feed-ingester/internal/automap/resolver.go
// `CacheCompetitorProfile`).
//
...
// `competitors.logo_url`. It's the canonical pipeline now — Wikipedia /
// Liquipedia were workarounds while we hadn't realised Oddin already had
// us covered. Coverage on prod: 1861 / 2135 cached profiles have an
// icon_path set (~87 %).
//
...
//     sh -c "pnpm db:resolve-oddin-icons"
//
// Flags:
